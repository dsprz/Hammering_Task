...
struct Hit_Task : mc_control::fsm::State
{
  ...
  private:
    ...
    // ROS
    mc_rtc::NodeHandlePtr _nh = nullptr;
    rclcpp::Subscription<geometry_msgs::msg::Vector3Stamped>::SharedPtr _subForce = nullptr;


    // Forces
    vector3_t _force_vector_on_nail;
    double _max_force_norm_on_nail = 0;
    double _max_normal_force_norm_on_nail = 0;

    vector3_t _peak_force_on_nail = {0, 0, 0};
    vector3_t _peak_normal_force_on_nail = {0, 0, 0};
    ...
    rbd::MultiBody _robot_multibody; 
    std::vector<std::vector<double>> _robot_initial_configuration; 
    int _nrdof = 1;
    std::shared_ptr<mc_solver::DynamicsConstraint> _dynamicsConstraint;
    rbd::MultiBodyConfig _final_config;
